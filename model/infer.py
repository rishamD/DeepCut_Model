...
class RecommendationEngine:
    ...
    def build_movie_index(self, movies_df: pd.DataFrame):
        ...
        for start in tqdm(
            range(0, total_movies, batch_size), desc="Encoding Movies"
        ):
            chunk = movies_df.iloc[start : start + batch_size]

            movie_ids = self.vocab.safe_encode(
                self.vocab.movie_encoder, chunk["movie_id"]
            )
            movie_language = self.vocab.safe_encode(
                self.vocab.language_encoder,
                chunk["original_language"].fillna("unknown"),
            )
            movie_genres = np.stack(
                chunk["genres"].apply(self.vocab.encode_genres_multihot).values
            )

            batch = {
                "movie_id": torch.tensor(
                    movie_ids, dtype=torch.long
                ).to(self.device),
                "movie_continuous": torch.tensor(
                    chunk[
                        ["popularity_scaled", "runtime_scaled", "year_scaled"]
                    ].values,
                    dtype=torch.float32,
                ).to(self.device),
                "movie_language": torch.tensor(
                    movie_language, dtype=torch.long
                ).to(self.device),
                "movie_genres": torch.tensor(
                    movie_genres, dtype=torch.float32
                ).to(self.device),
            }

            with torch.no_grad():
                chunk_emb = (
                    self.model.get_movie_embedding(batch).cpu().numpy()
                )
    ...
    def recommend(
        ...
        lang = self.vocab.safe_encode(
            self.vocab.language_encoder,
            [user_features.get("preferred_language", "unknown")],
        )[0]

        batch = {
            "user_id": torch.tensor(
                [uid], dtype=torch.long
            ).to(self.device),
            ...
            "user_lang": torch.tensor(
                [lang], dtype=torch.long
            ).to(self.device),
        }
